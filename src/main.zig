...
fn dbDemo(ctx: *zzz.Context) !void {
    ...
    // Convert to template-friendly views (strings only)
...
fn renderStatsHtml(buf: *[2048]u8) []const u8 {
    ...
    // Show last 10 log entries (newest first)
...
fn jobsEnqueue(ctx: *zzz.Context) !void {
    ...
    // Map to string literals — MemoryStore holds slices, so the worker
    // name must outlive the HTTP request buffer.
